...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'signal_labels' AND policyname = 'service_role_all'
  ) THEN
    CREATE POLICY "service_role_all" ON public.signal_labels
      FOR ALL TO service_role USING (true) WITH CHECK (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'signal_labels' AND policyname = 'read_public'
  ) THEN
    CREATE POLICY "read_public" ON public.signal_labels
